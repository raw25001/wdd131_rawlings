...
function createGrid() {
    ...
    characters.forEach((char, index) => {
        ...
        tile.innerHTML = `
            ...
            <h4>${char.name}</h4>
            <input type="number" min="0" max="20" value="0" id="num-${index}">
        `;
...
function updateResults() {
    ...
    for (let key in mechanicsMap) {
        mechanicsMap[key].clear();
    }
    ...
    characters.forEach((char, index) => {
        ...
        if (value > 0) {
            mechanicsMap[char.category].add(true);
        }
    });

    document.getElementById("score").textContent = `Score: ${total * 10}`;

    const list = document.getElementById("mechanicsList");
    list.innerHTML = "";

    Object.keys(mechanicsMap).forEach(mech => {
        if (mechanicsMap[mech].size > 0) {
